...
***************************************************************************** */
        ...
        if (audioBlocks.length > 0) audioBlocks.forEach((audioBlock => {
            ...
            player.querySelector(".player__time");
            const timeCurrent = player.querySelector(".player__time_current");
            const timeDuration = player.querySelector(".player__time_diration");
            let isMoving = false;
            init();
            function init() {
                audio.addEventListener("loadeddata", (() => setTimeInHTML(timeDuration, audio.duration)));
            ...
            function setTimeInHTML(timeBlock, timeSeconds) {
                ...
                timeBlock.innerHTML = `${minutes >= 10 ? minutes : "0" + String(minutes)}:${seconds >= 10 ? seconds : "0" + String(seconds)}`;
            }
            ...
            function updateSongProgress(clientX, shiftX = 0) {
                ...
                if (newLeft <= 0) audio.currentTime = 0; else if (newLeft >= progressBar.offsetWidth - thumb.offsetWidth) {
                    if (audio.duration) audio.currentTime = audio.duration;
                } else audio.currentTime = newLeft * audio.duration / progressBar.offsetWidth;
            }
            ...
            function updateProgressBar() {
                ...
                let widthPerSecond = audioDuration > 0 ? barWidth / audioDuration : 0;
                let widthPerSecondInPercent = audioDuration > 0 ? widthPerSecond * 100 / barWidth : 0;
                thumb.style.transform = `translate(${widthPerSecond * audio.currentTime - thumb.offsetWidth / 2}px, -50%)`;
            ...
            if (musicLinks.length > 0) {
                ...
                musicLinks.forEach((musicLink => {
                    musicLink.addEventListener("click", (() => {
                        ...
                        pauseMusic();
                        removeActiveMusicLink(musicLink);
                        playMusic();
                    }));
                }));
                ...
                function endedMusic(e) {
                    if (!isMoving) {
                        ...
                        if (nextMusicLink) {
                            pauseMusic();
                            removeActiveMusicLink(nextMusicLink);
                            playMusic();
                        } else {
                            pauseMusic();
                            removeActiveMusicLink(musicLinks[0]);
                            updateProgressBar();
                        }
                    }
                }
                function removeActiveMusicLink(newActiveLink) {
