...
***************************************************************************** */
        ...
        let audioBlocks = document.querySelectorAll("[data-audio]");
        if (audioBlocks.length > 0) audioBlocks.forEach((audioBlock => {
            const musicLinks = audioBlock.querySelectorAll("[data-src]");
            const player = audioBlock.querySelector("[data-player]");
            ...
            const progress = progressBar.querySelector(".player__progress");
            const thumb = progressBar.querySelector(".player__thumb");
            const time = player.querySelector(".player__time");
            const timeCurrent = time.querySelector(".player__time_current");
            const timeDuration = time.querySelector(".player__time_diration");
            let isMoving = false;
            init();
            function init() {
                setTimeInHTML(timeDuration, audio.duration);
                setTimeInHTML(timeCurrent, audio.currentTime);
                audio.addEventListener("loadedmetadata", (() => {
                    setTimeInHTML(timeDuration, audio.duration);
                    setTimeInHTML(timeCurrent, audio.currentTime);
                }));
            }
            function setTimeInHTML(timeBlock, timeSeconds) {
                let minutes = timeSeconds ? Math.floor(timeSeconds / 60) : 0;
                let seconds = timeSeconds ? Math.floor(timeSeconds % 60) : 0;
                if (timeSeconds) {
            ...
            audio.addEventListener("pause", (() => playButton.classList.remove("_play")));
            audio.addEventListener("play", (() => playButton.classList.add("_play")));
            ...
            function pauseMusic(currentAudio = audio, currentPlayBtn = playButton) {
                currentAudio.pause();
                currentPlayBtn.classList.remove("_play");
            }
            function pauseAllMusic() {
                audioBlocks.forEach((audioBlock => {
                    const audio = audioBlock.querySelector(".player__audio");
                    const playButton = audioBlock.querySelector(".player__play-btn");
                    pauseMusic(audio, playButton);
                }));
            }
            progressBar.addEventListener("click", (e => {
                e.preventDefault();
                updateSongProgress(e.clientX);
            }));
            if (isMobile.any()) thumb.addEventListener("touchstart", (e => {
                if (e.cancelable) e.preventDefault();
                let shiftX = e.touches[0].clientX - e.target.getBoundingClientRect().left;
                document.addEventListener("touchmove", onTouchMove);
                document.addEventListener("touchend", onTouchEnd);
                const isAudioPausedBeforeMove = audio.paused;
                if (!isAudioPausedBeforeMove) pauseMusic();
                isMoving = true;
                function onTouchMove(e) {
                    updateSongProgress(e.touches[0].clientX, shiftX);
                }
                function onTouchEnd() {
                    document.removeEventListener("touchmove", onTouchMove);
                    isMoving = false;
                    if (!isAudioPausedBeforeMove) playMusic();
                    document.removeEventListener("touchend", onTouchEnd);
                }
            })); else thumb.addEventListener("mousedown", (e => {
                e.preventDefault();
                ...
                function onMouseMove(e) {
                    updateSongProgress(e.clientX, shiftX);
                }
                function onMouseUp() {
            ...
            function updateSongProgress(clientX, shiftX = 0) {
                let newLeft = clientX - progressBar.getBoundingClientRect().left - shiftX;
                if (!audio.duration) return;
                if (newLeft <= 0) audio.currentTime = 0; else if (newLeft >= progressBar.offsetWidth + thumb.offsetWidth) audio.currentTime = audio.duration; else audio.currentTime = newLeft * audio.duration / progressBar.offsetWidth;
            }
            ...
            function updateProgressBar() {
                let barWidth = progressBar.offsetWidth - thumb.offsetWidth;
                let audioDuration = audio.duration;
                let widthPerSecond = 0;
                let widthPerSecondInPercent = 0;
                let widthPerSecondForThumb = 0;
                if (audioDuration) {
                    widthPerSecond = barWidth / audioDuration;
                    widthPerSecondInPercent = widthPerSecond * 100 / barWidth;
                    widthPerSecondForThumb = (barWidth + thumb.offsetWidth) / audioDuration;
                }
                thumb.style.transform = `translate(${widthPerSecondForThumb * audio.currentTime - thumb.offsetWidth / 2}px, -50%)`;
                progress.style.width = widthPerSecondInPercent * audio.currentTime + "%";
            }
            window.addEventListener("resize", updateProgressBar);
            if (musicLinks.length > 0) {
                audio.src = musicLinks[0].dataset.src;
                musicLinks[0].classList.add("_active");
                musicLinks.forEach((musicLink => {
                    musicLink.addEventListener("click", (() => {
                        if (audio.src === musicLink.dataset.src) return;
                        pauseAllMusic();
                        nextActiveMusic(musicLink);
                        playMusic();
                    }));
                }));
                audio.addEventListener("ended", (e => endedMusic(e)));
                function endedMusic(e) {
                    if (!isMoving) {
                        let nextMusicLink = audioBlock.querySelector(`[data-src="${e.currentTarget.getAttribute("src")}"]`).nextElementSibling;
                        if (nextMusicLink) {
                            pauseMusic();
                            nextActiveMusic(nextMusicLink);
                            playMusic();
                        } else {
                            pauseMusic();
                            nextActiveMusic(musicLinks[0]);
                            updateProgressBar();
                        }
                    }
                }
                function nextActiveMusic(newActiveLink) {
                    musicLinks.forEach((musicLink => musicLink.classList.remove("_active")));
                    newActiveLink.classList.add("_active");
                    audio.src = newActiveLink.dataset.src;
                }
            }
        }));
        const menuItemsGoTo = document.querySelectorAll(".header__menu .menu__item [data-goto]");
        const gotoBlocks = [];
        menuItemsGoTo.forEach((item => {
            const gotoBlock = document.querySelector(item.getAttribute("data-goto"));
            gotoBlocks.push(gotoBlock);
        }));
        ...
        window.addEventListener("scroll", (() => {
            ...
            if (headerElement.classList.contains("_header-scroll")) headerItemHeight = headerElement.offsetHeight;
            gotoBlocks.forEach((block => {
                let blockTop = block.offsetTop;
                let blockBottom = block.offsetTop + block.offsetHeight;
                let toTop = document.querySelector(`.header__menu .menu__item [data-goto='#${block.id}']`).getAttribute("data-goto-top") ? parseInt(document.querySelector(`.header__menu .menu__item [data-goto='#${block.id}']`).getAttribute("data-goto-top")) : 0;
                if (activeBlock === block && blockTop - headerItemHeight - toTop > scrollY) {
                    if (activeMenuItem) activeMenuItem.classList.remove("_active");
                    activeBlock = null;
                    activeMenuItem = null;
                } else if (scrollHeight - scrollY - window.innerHeight <= block.offsetHeight / 100 * 20 || blockTop - headerItemHeight - toTop <= scrollY && blockBottom - headerItemHeight >= scrollY && activeBlock !== block) {
                    if (activeMenuItem) activeMenuItem.classList.remove("_active");
                    activeBlock = block;
                    activeMenuItem = document.querySelector(`.header__menu .menu__item [data-goto='#${activeBlock.id}']`);
                    activeMenuItem.classList.add("_active");
                }
            }));
        }));
        let isLessTablet = false;
        changeTraksImagePlace();
        changeTracksListWrapperHeight();
        window.addEventListener("resize", (() => {
            changeTraksImagePlace();
            changeTracksListWrapperHeight();
        }));
        function changeTraksImagePlace() {
            if (document.documentElement.offsetWidth >= 992 && isLessTablet) {
                const traksBlock = document.querySelector(".tracks");
                const traksImage = traksBlock.querySelector(".tracks__image");
                traksBlock.querySelector(".tracks__body").prepend(traksImage);
                isLessTablet = false;
            } else if (document.documentElement.offsetWidth < 992 && !isLessTablet) {
                const traksBlock = document.querySelector(".tracks");
                const traksImage = traksBlock.querySelector(".tracks__image");
                traksBlock.querySelector(".tracks__title").after(traksImage);
                isLessTablet = true;
            }
        }
        function changeTracksListWrapperHeight() {
            const tracksListWrapper = document.querySelector(".tracks .tracks__list-wrapper");
            const tracksItems = tracksListWrapper.querySelectorAll(".tracks__item");
            let showItemsCount = tracksListWrapper.getAttribute("data-count-traks-item");
            if (tracksItems.length < showItemsCount) showItemsCount = tracksItems.length;
            let height = 5;
            for (let i = 0; i < showItemsCount; i++) {
                const item = tracksItems[i];
                const style = item.currentStyle || window.getComputedStyle(item);
                const matrginBottom = i !== showItemsCount - 1 ? parseInt(style.marginBottom) : 0;
                height += item.offsetHeight + matrginBottom;
            }
            tracksListWrapper.style.height = height + "px";
        }
        window["FLS"] = true;
        isWebp();
        addLoadedClass();
